...
fn combine_occurences<'a>(
    ...
    b.into_iter().fold(a, |mut hash_map, (word, count)| {
        // the `mut hash_map` is the accumulator initialized as `a`.
        // the (word, count) tuple is the .next() key, value pair in `b`.
        match (hash_map.contains_key(word), count > 0) {
            (true, true) => _ = hash_map.entry(word).and_modify(|c| *c += count),
            (false, true) => _ = hash_map.insert(word, count),
            _ => (),
        };
        hash_map
    })
